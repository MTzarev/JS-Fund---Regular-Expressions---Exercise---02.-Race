function race(input){
    ...
    while(command !== "end of race"){
        ...
        let matchDistance = command.match(distancePattern).map(Number).reduce((sum,el) => sum + el);
                if(sportalsNames.includes(matchesName) && !racers.hasOwnProperty(matchesName)){
            racers[matchesName] = matchDistance     ;
    }else if(sportalsNames.includes(matchesName) && racers.hasOwnProperty(matchesName)){
        racers[matchesName] += matchDistance;
    }
    command = input.shift();
    }
